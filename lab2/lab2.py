...
def set_attacked(chessboard: list, x: int, y: int, n: int) -> None:  # установка атаки фигуры, везде проверяя, что не выходим за границы доски
...
def search_solutions(chessboard: list, n: int, l: int, k: int, x_prev: int, y_prev: int) -> None:
    global IS_FINDED
        ...
        for y in range(y1, n):
            if chessboard[x][y] == FREE:  # если поле свободно, то
                ...
                if k + 1 == l:  # если кол-во расставленных фигур равно L, то
                    output(board, n)  # вывод расстановки в файл
                    IS_FINDED += 1  # флаг, что хоть одна расстановка найдена
                    if IS_FINDED == 1:
                        print_board(board, n)  # вывод доски в консоль
                else:
                    search_solutions(board, n, l, k + 1, x, y)  # вызов ф-ции для следующей установки фигуры
...
def main():
    ...
    if L == 0:
        ...
        search_solutions(chessboard, N, L, 0, 0, 0) #поиск всех возможных расстановок
